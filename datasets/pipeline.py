...
class Normalize:
    ...
    def __call__(self, results):
        ...
        if modality == 'RGB':
            ...
            # assert imgs.shape == (16, 224, 224, 3), "Error: images.shape is not (8, 224, 224, 3)"
            for img in imgs:
...
class DecordInit:
    ...
    def __call__(self, results):
        ...
        # container = decord.VideoReader(file_obj, num_threads=self.num_threads) "/data/lql/zjl_86_torch/Erasing-Attention-Consistency-main/data/oulu_image_base/0/P001/FirstF_000.jpeg"
        container = None
        results['video_reader'] = container
        # results['total_frames'] = len(container)
        results['total_frames'] = int(results['num_frames'])  ##帧数
        return results
...
def get_frame_index(num_frames, seg, num_segments):
...
class DecordDecode:
    ...
    def __call__(self, results):
        ...
        # file_name = results['filename'].split('/')[-1].split('.')[0]
        label = str(results['label'])
        # image_root = "/data/lql/zjl_86_torch/Erasing-Attention-Consistency-main/data/oulu_image_base/"
        # image_root = "/data/lql/zjl_86_torch/Erasing-Attention-Consistency-main/data/DFEW/DFEW_align_images/"
        # image_path = os.path.join(image_root, label)
        image_path = results['filename']
        if '.avi' in image_path:
        ...
        # image_path_MV = image_path.replace(
        #     "/data/zjl/192-torch/pytorch-coviar-master/data/DFEW-compress/DFEW_image_align_R_2/",
        #     "/data/zjl/192-torch/pytorch-coviar-master/data/DFEW-compress/DFEW_image_align_MV_not20_2/")
        # image_list_MV = os.listdir(image_path_MV)
        # image_list_MV.sort(key=lambda x: int(x.replace("FirstF_", "").split('.')[0]))
        #
        # imgs_MV = []
        ###Oulu
        # file_name = results['filename'].split('/')[-1].split('.')[0]
        ...
        for idx in frame_inds:
            if idx >= len(image_list):
                # print(f"image_len:{len(image_list)}, idx: {idx}")
                idx = len(image_list) - 1

            image_name = image_list[idx]
            image_name_path = os.path.join(image_path, image_name)
            image_file = cv2.imread(image_name_path)
            image_np = np.array(image_file)
            imgs.append(image_np)
            #
            # image_name_MV = image_list_MV[idx]
            # image_name_path_MV = os.path.join(image_path_MV, image_name_MV)
            # image_file_MV = cv2.imread(image_name_path_MV)
            # image_np_MV = np.array(image_file_MV)
            # imgs_MV.append(image_np_MV)

        # imgs = imgs + imgs_MV

        ###自定义的帧数选择
        # for seg in range(len(frame_inds)) :
        #
        #     image_name_index = get_frame_index(num_frames=len(image_list), seg=seg, num_segments=8)
        #     image_name = image_list[image_name_index]
        #     image_name_path = os.path.join(image_path, image_name)
        #     image_file = cv2.imread(image_name_path)
        #     image_np = np.array(image_file)
        #     imgs.append(image_np)
...
class Collect:
    ...
    def __init__(self,
                 ...
                 nested=False):
        # self.keys = keys
        self.keys = ['imgs', 'label', 'filename', 'frame_inds']
