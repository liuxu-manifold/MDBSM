...
def parse_option():
    ...
    parser.add_argument('--config', '-cfg', required=False, type=str,
                        default="/data1/liuxu/lx_190/MDBSM/configs/k400/16_16.yaml")
    parser.add_argument(
        "--opts",
    ...
    parser.add_argument('--output', type=str, default="/data1/liuxu/lx_190/MDBSM/Output/")
    ...
    ####MAFW
    # 注释掉的代码：设置MAFW数据集的训练和测试txt文件路径
    # args.train_txt_path = f"/data1/liuxu/dataset/video-dataset/dataset-txt/MAFW-txt/set_{flod_id}_train.txt"
    # args.test_txt_path = f"/data1/liuxu/dataset/video-dataset/dataset-txt/MAFW-txt/set_{flod_id}_test.txt"

    ####FER39k
    # args.train_txt_path = "/data1/liuxu/dataset/video-dataset/dataset-txt/FER39k-txt/train.txt"
    # args.test_txt_path = "/data1/liuxu/dataset/video-dataset/dataset-txt/FER39k-txt/test.txt"


    ####DFER
    # 设置DFER数据集的训练和测试txt文件路径
    args.train_txt_path = f"/data1/liuxu/dataset/video-dataset/dataset-txt/DFEW-txt/DFEW_set_{flod_id}_train.txt"
    args.test_txt_path = f"/data1/liuxu/dataset/video-dataset/dataset-txt/DFEW-txt/DFEW_set_{flod_id}_test.txt"

    # 获取当前脚本（main.py）的绝对路径
    current_script_path = os.path.abspath(__file__)
    # 获取当前脚本所在的目录
    current_script_dir = os.path.dirname(current_script_path)
    # 拼接脚本目录下的 Output 文件夹路径
    output_dir = os.path.join(current_script_dir, "Output")
    # 赋值给 args.save_txt_path
    args.save_txt_path = output_dir
...
def main(config):
    ...
    for epoch in range(start_epoch, config.TRAIN.EPOCHS):
        ...
        # if is_best:
        #     epoch_saving(config, epoch, model.module, max_accuracy, optimizer, lr_scheduler, logger, config.DATA.SAVE_CHEK_FILE,
        #                  is_best)
...
def print_confusion_matrix(conf_matrix, config, name):
    ...
    plt.colorbar()
    # labels = ["Ha", 'Sa', 'Ne', 'Ar', 'Su', 'Disg', 'Fe', 'Co', 'An', 'He', 'Disa']  ####每种类别的标签   MAFW
    labels = ["Ha", 'Sa', 'Ne', 'Ar', 'Su', 'Di', 'Fe']  # 每种类别的标签  DFEW/ FER39K
    # labels = ["Ha", 'Sa', 'Ne', 'Ar', 'Su', 'Di', 'Fe']  # 每种类别的标签  DFEW/ FER39K
