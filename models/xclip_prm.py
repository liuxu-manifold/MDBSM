...
from cct_prm import CrossFrameCommunicationTransformer
import sys
import warnings
sys.path.append("../")
...
class XCLIP(CLIP):
    ...
    def encode_video(self, image):
        ...
        cls_features, img_features, beta = self.encode_image(image)
    ...
    def forward(self, image, text):
        ...
        video_logits = torch.einsum("bd,bkd->bk", video_features, logit_scale * text_features)
        frame_logits = torch.einsum("btd,bkd->btk", frame_features, logit_scale * text_features)
        
        return video_logits, frame_logits, q, beta
