...
def fix_text(model):
    # for name, param in model.named_parameters():
    #     if "visual." in name or "mit" in name or "prompts" in name or "fta" in name or "st_" in name or "_MV" in name or "_Adapter" in name:
    #         continue
    #     else:
    #         param.requires_grad=False

    # for name, param in model.named_parameters():
    #     if "_Adapter" in name or "mit" in name or "prompts" in name or "fta" in name or "st_" in name or "_MV" in name:
    #         continue
    #     else:
    #         param.requires_grad=False
    ...
    for name, param in model.named_parameters():
        ...
        if any(k in name for k in ["_Adapter", "prm_adapter", "mcp_adapter", "temporal_reliability",
                                   "mit", "prompts", "fta", "message_", "mamba_", "mdm", "bsm", "msa"]):
            continue
        else:
            param.requires_grad = False
...
def build_optimizer(config, model):
    ...
    st_parameters = set_weight_decay(model, skip, skip_keywords,
                                     weight_decay=config.TRAIN.WEIGHT_DECAY, lr=config.TRAIN.LR * 10,
                                     have=("st_",), not_have=()
                                     )
    adp_parameters = set_weight_decay(model, skip, skip_keywords,
                                      weight_decay=config.TRAIN.WEIGHT_DECAY, lr=config.TRAIN.LR * 10,
                                      have=("Adapter",), not_have=()
                                      )
